...
public class B_ArrayCopy {
	...
	public void method3() {
		...
		int[] origin = { 1, 2, 3, 4, 5 };
		int[] copy = new int[10]; // 넉넉하게 크게 생성 / 0~9 인덱스, 0이 다 담겨있을것(초기화를 안해서)
	...
	public void method4() {
		 ...
		 * 클래스들을 쓸때는 import 안해도됨
		 * 
		 * java.until.Scanner java.until.Arrays =>java.lang 이외의 패키지에 있는 클래스들을 쓸때는 무조건
		 * import 해야됨
		 */
		for (int i = 0; i < copy.length; i++) {
			System.out.print(copy[i] + " ");
		...
		System.out.println();
		/*
		 * Arrays.copyOf()메소드 =>내가 제시한 길이 만큼 복사본 배열 크기 할당 =>단, 내가 제시한 길이가 원본배열의 크기보다 클
		 * 경우 기존의 배열에 있는 값은 다 복사 후 나머지는 0으로 채운다. => 내가 제시한 길이가 원본배열의 크기보다 작을 경우 기존의 배열에
		 * 있는 값중 내가 제시한 길이만큼만 복사한다.
		 */
		System.out.println("origin주소값 : " + origin.hashCode());
		System.out.println("copy주소값 : " + copy.hashCode());
	}
	...
	public void method5() {
		// 4.clone() 메소드 이용한 복사
		// 인덱스 지정불가, 크기 지정 별도로 못함(쌍둥이 처럼 만든다.)
		int[] origin = { 1, 2, 3, 4, 5 };
		// 복사본 배열 = 원본배열.clone();
