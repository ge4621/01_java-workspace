...
public class D_Cast {
   ...
   * 
   * *형변환 종류
   * 1. 자동 형변환 => 자동으로 형변환이 이뤄져서 우리가 형변환을 시킬 필요가 없다.
   * 2. 강제(명시적) 형변환 =>자동 형변환이 안돼서 우리가 직접 형변환을 해줘야한다. 
   * 
   *  [표현범] (바꿀자료형)값
   *  
   *  *주의사항
   ...
   */
	...
	public void autoCasting() {  //메소드
		
		/*
		 * 자동형변환 (오른쪽이 왼쪽보다 작아야 한다.)
		...
		//엄청 큰 박스에는 작은 박스를 담을 수 있다. => 자동형변환 가능!
		...
		float f4 = 1.0f;
		double d4 = /*(doouble)*/f4;
		
		//====특이 케이스====
		//4. long(8byte) => float(4byte)
		//   float이 실수형이기 때문에 long형보다 표현가능한 수의 범위가 더 커서 자동형변환이 된다.!!
		...
		//byte b3 = b1 + b2;  //byte나 short는 연산시 무조건 int형으로 처리함.
							//연산 결과가 범위가 더 큰 int형임. => byte형에 대입 불가능
		byte b3 = (byte)(b1 + b2);
	...
	public void forceCasting() {
		...
		double d1 = 4.0;
		float f1 = (float)d1;
		//강제형변환을 안해주면 오류남!!명시적으로 형변환 해줘야함
