...
public class A_Array {
	...
	public void method1() {
		 ...
		 * 
		 * //출력하고자 할때도 => 일일이 출력해야 됨(반복문 활용 불가) System.out.println(num1);
		 * System.out.println(num2); System.out.println(num3); System.out.println(num4);
		 * System.out.println(num5);
		 * 
		 * for(int i = 1;i<=5;i++) { System.out.println(numi); //안됨 } //총합계를 구할 때도 =>
		 * 일일히 더해줘야 함(반복문 활용 불가) int sum = num1 + num2 + num3 + num4 + num5;
		 */

		/*
		 * 1.배열 선언(여려개의 값을 보관할 배열을 만들겠다.!!) [표현법] 자료형 배열명[]; 자료형[] 배열명; => 저는 이걸로
		 * 갑니다.(일반적으로 많이 쓰는 방법)
		 */
		// int a;//변수 선언
		...
		/*
		 * 2. 배열 할당(이 배열에 몇개의 값들을 보관할껀지 크기를 지정하는 과정, 지정 개수만큼 방이 만들어진다.) [표현법] 배열명 = new
		 * 자료형[배열크기];
		 */
		// arr = new int[5];
		...
		System.out.println(arr); // 실제 arr이라는 박스에는 "주소값"이라는게 담겨 있음(뭔소리지?=>method2())
	...
	public void method2() {
		...
		/*
		 * 실제 리터럴(=>값)을 곧바로 담을수 있는 변수를 일반변수라고 이야기한다. 주소값을 담고있는 변수는 참조변수(=>레퍼런스 변수)라고
		 * 이야기한다.
		 * 
		 * 기본 자료형(boolean,char,byte,short,int,long,float,double)으로 선언된 변수 : 소문자로 시작하여
		 * 소문자로 끝난다. => 실제 리터럴 값을 바로 담는 변수 => 그냥 일반 변수
		 * 
		 * 그 외 자료형(int[],double[], char[],short[],요런 자료형, String,....)으로 선언된 변수 =>주소값을
		 * 담는 변수 => 참조변수(레퍼런스변수)
		 * 
		 */
