		...
		while (flag == true) { // 메뉴 스위치가 켜져있는 동안 무한반복
			...
			case 1: // 1. 조회하기
				...
				if(bList.isEmpty()) {
					...
					ArrayList<Book> tempB = bc.onlySearchBook();
					...
					ArrayList<Book> tempM = bc.onlySearchMagazine();
					for(int i = 0; i<tempM.size();i++) {
						System.out.println(tempM.get(i));
					}
			...
			case 4:// 4. 전체책 가격 합계 및 평균 조회
				System.out.println("전체책 가격 합계 : " + bc.getTotalPrice() + "원 입니다." );
				System.out.println("전체책 가격 평균 : " + bc.getAvgPrice() + "원 입니다.");
				break;
